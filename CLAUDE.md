...
## Per-app internals
- **531-bbb**: `route` var + `render()`/`go()` swap views (Setup/Home/Train/History/Progress). State (`settings`/`state`/`history`) in `localStorage` via `load()`/`save()`. Plate math: `plateBreakdown`, `diffPlateCounts`, `buildSessionPlateStages`. `history.json` is a static export, unused at runtime. Home's Cycle Progress grid (`renderCycleProgress()`) is the top visual: each cell shows the logged top-set weight/reps if that lift's done, "Next" if it's the current slot, else a dash, and is clickable (`doCycleCellClick`) — a done cell opens `editingHistoryIdx` (the existing edit-history modal), an empty one jumps `state.week`/`dayIndex` straight into Train. The "Next workout" card + Start/pick-day buttons sit below it. Train's set/BBB weights are editable (`sessionWeights`, lazily seeded by `ensureSessionWeights()` from the computed prescription, reset on every `go()`) — editing a `.weight-input`/`#bbbWeightInput` updates `sessionWeights` and re-renders so `buildSessionPlateStages` recomputes live off the edited values; `doFinishWorkout()` logs `sessionWeights`, not the raw computed ones.
