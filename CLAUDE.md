...
Each app is **one self-contained `index.html`** (inline `<style>`+`<script>`, no external JS/npm). Only external resources: Google Fonts `<link>`s; `rally-clipper` also lazy-loads ffmpeg.wasm + an optional ML model.
